from sparse_mkl._mkl_interface import (MKL, sparse_matrix_t, _create_mkl_sparse, debug_print, debug_timer,
...
def _sparse_add_sparse(matrix_a, matrix_b, cast=False, reorder_output=False, dense=False):
    """
    ...
    double_precision = a_dbl or b_dbl
    # Allocate an array for outputs and set functions and types for float or doubles
    func = MKL._mkl_sparse_d_add if double_precision else MKL._mkl_sparse_s_add
    alpha = _ctypes.c_double(1) if double_precision else _ctypes.c_float(1)

    ref_handle = sparse_matrix_t()
    ret_val = func(_ctypes.c_int(10),
                   mkl_a,
                   alpha,
                   mkl_b,
                   _ctypes.byref(ref_handle))
    mkl_c = ref_handle
    # Check return
    _check_return_value(ret_val, "_mkl_sparse_add")
